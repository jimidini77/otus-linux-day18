...
Контейнеры используют ядро хостовой системы. Собрать ядро внутри контейнера можно, но применить для контейнера нельзя.
...
помещены в публичный репозиторий. После разворачивания машины запущены 3 контейнера; на порту `tcp:80` доступен контейнер из кастомного образа nginx, 
на порту `tcp:81` доступен контейнер созданный docker-compose. Образ nginx загружен в DockerHub:
